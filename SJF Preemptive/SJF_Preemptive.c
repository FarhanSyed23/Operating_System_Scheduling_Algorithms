...
void main()
{
        int i,j,n,burst[x],wait[x],turn[x],temp[x],process[x],arrival[x],total=0,sum=0,position; // total is the sum of waiting times, sum is the sum of turnaround times, temp is for swapping values, position is for comparing
        ...
        for(i=0;i<n;i++)
        {
            ...
            total=total+wait[i];
                  ...
                  // total is the summation of the waiting times
       ...
       /* for(i=0;i<n;i++)
        ...
        for(i=0;i<n;i++)
        {
            ...
            printf("\nP%d\t\t%d\t\t%d\t\t%d\t\t%d\n",process[i],burst[i],arrival[i],wait[i],turn[i]);
        }
